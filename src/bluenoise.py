...
def bandlimited_blue_noise(height, width, sigma_map, levels = 5, r_px = 12, seed = 0, sigma_blob = 0.5):
    # Generate base blue noise field
    field = blue_noise_field_from_poisson_points(height, width, r_px = r_px, seed = seed, sigma_blob = sigma_blob)
    
    # Create pyramids
    Gn = make_gaussian_pyramid(field, levels)
    Ln = make_laplacian_pyramid(field, levels, gaussian_pyramid = Gn)
    
    # Resize all Laplacian bands to full resolution
    full = []
    for l in range(levels - 1):
        full.append(cv2.resize(Ln[l].astype(np.float32), (width, height), interpolation = cv2.INTER_CUBIC))
    full = np.stack(full, axis = 0)
    
    # Select appropriate band based on sigma_map
    ...
    # Normalize to unit variance (like your original)
    result -= result.mean()
